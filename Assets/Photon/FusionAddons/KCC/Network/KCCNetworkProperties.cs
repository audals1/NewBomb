namespace Fusion.Addons.KCC
{
	...
	public sealed unsafe class KCCNetworkProperties : KCCNetworkProperty<KCCNetworkContext>
	{
		...
		public override void Write(int* ptr)
		{
			...
			Vector3 positionExtension = fullPrecisionPosition - networkBufferPosition;

			bool keepAlive = true;

			keepAlive &= CompareAndWriteVector3(positionExtension, ref ptr);
			keepAlive &= CompareAndWriteFloat(data.LookPitch, ref ptr);
			keepAlive &= CompareAndWriteFloat(data.LookYaw, ref ptr);
			...
			combinedSettings |= ((int)settings.ProxyBehavior          & 0b11)    << 17;
			...
			{
				int size   = 0b111;
				int number = (*ptr >> 29) & size;

				combinedSettings |= (((number + 1) % size) & size) << 29;
			}
		...
		public override void Interpolate(KCCInterpolationInfo interpolationInfo)
		{
			...
			int combinedSettings = InterpolateInt(ref interpolationInfo);
			...
			settings.CollisionLayerMask = InterpolateInt(ref interpolationInfo);

			settings.Radius = InterpolateFloat(ref interpolationInfo);
			settings.Height = InterpolateFloat(ref interpolationInfo);
			settings.Extent = InterpolateFloat(ref interpolationInfo);
			...
			InterpolateInteractions(runner, data, _maxTotalInteractions, _interactionsWordCount, ref interpolationInfo);
		}
		...
		private static void InterpolateInteractions(NetworkRunner runner, KCCData data, int maxTotalInteractions, int interactionsWordCount, ref KCCInterpolationInfo interpolationInfo)
		{
			if (maxTotalInteractions <= 0)
			{
				interpolationInfo.Offset += interactionsWordCount;
				return;
			}

			NetworkBehaviourBuffer buffer = interpolationInfo.Alpha < 0.5f ? interpolationInfo.FromBuffer : interpolationInfo.ToBuffer;
			int                    offset = interpolationInfo.Offset;

			int  interactionCount = buffer.ReinterpretState<int>(offset); ++offset;
			int  collisionCount   = (interactionCount >> (INTERACTIONS_BITS_SHIFT * 0)) & INTERACTIONS_MASK_SINGLE;
			int  modifierCount    = (interactionCount >> (INTERACTIONS_BITS_SHIFT * 1)) & INTERACTIONS_MASK_SINGLE;
			int  ignoreCount      = (interactionCount >> (INTERACTIONS_BITS_SHIFT * 2)) & INTERACTIONS_MASK_SINGLE;
			...
			for (int i = 0; i < collisionCount; ++i)
			{
				KCCNetworkID networkID = KCCNetworkUtility.ReadNetworkID(buffer, offset);
				offset += KCCNetworkID.WORD_COUNT;
