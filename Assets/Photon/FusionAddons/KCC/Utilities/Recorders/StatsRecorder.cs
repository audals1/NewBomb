namespace Fusion.Addons.KCC
{
    ...
    /// <item><description>Frame          - Recorded graph is driven by <c>Time.frameCount</c> - file ends with (Frame).</description></item>
    /// <item><description>EngineTime     - Recorded graph is driven by <c>Time.unscaledTime</c> - file ends with (EngineTime).</description></item>
    /// <item><description>MonitorTime    - Recorded graph is driven by simulated monitor time - file ends with (MonitorTime).</description></item>
    /// <item><description>SimulationTick - Recorded graph is driven by <c>Runner.Tick</c> - file ends with (SimulationTick).</description></item>
    /// </list>
	/// </summary>
	...
	public abstract class StatsRecorder : NetworkBehaviour, IBeforeUpdate, IBeforeAllTicks, IAfterTick
	{
		// PUBLIC MEMBERS
		...
		[SerializeField][DisabledInPlayMode]
		private ERecorderType _recorderType = ERecorderType.EngineTime;
		[SerializeField][DisabledInPlayMode]
		private int           _referenceRefreshRate = 0;
		...
		public virtual bool IsSupported(ERecorderType recorderType) => true;
		...
		private void LateUpdate()
		{
			...
			else if (_activeRecorderType == ERecorderType.MonitorTime)
			{
				...
				while (TryRefreshMonitor() == true)
				{
					if (_statsWriter.HasValues == true)
					{
		...
		{
			if (_activeRecorderType == ERecorderType.SimulationTick && Runner.IsForward == true)
			{
				WriteValues();
			}
		}
		...
		private void Initialize(ERecorderType recorderType)
		{
			...
			int monitorRefreshRate = _referenceRefreshRate;
			...
			string fileSuffix    = _recorderType == ERecorderType.MonitorTime ? $"_{monitorRefreshRate}Hz" : default;
			string statsFileName = $"Rec_{fileID}_{name}_{GetType().Name}_{_activeRecorderType}{fileSuffix}.log";
