namespace Fusion.Addons.KCC
{
	...
	public class KCCRecorder : StatsRecorder
	{
		...
		protected KCC KCC { get { if (_kcc == null) { _kcc = GetComponent<KCC>(); } return _kcc; } }
		...
		protected override void GetHeaders(ERecorderType recorderType, List<string> headers)
		{
			...
			if (recorderType == ERecorderType.SimulationTick)
			{
				headers.Add("KCC Overlap Queries");
				headers.Add("KCC RayCast Queries");
				headers.Add("KCC ShapeCast Queries");
				headers.Add("KCC Fixed Timer [ms]");
			}
			else
			{
				headers.Add("KCC Fixed Updates");
				headers.Add("KCC Fixed Timer [ms]");
				headers.Add("KCC Render Timer [ms]");
				headers.Add("KCC Prediction Error");
			}
		}
		...
		protected override bool AddValues(ERecorderType recorderType, StatsWriter writer)
		{
			...
			if (recorderType == ERecorderType.SimulationTick)
			{
				writer.Add($"{KCC.Debug.OverlapQueries}");
				writer.Add($"{KCC.Debug.RayCastQueries}");
				writer.Add($"{KCC.Debug.ShapeCastQueries}");
				writer.Add($"{KCC.Debug.FixedTimer.GetRecentMilliseconds():F4}");
			}
			else
			{
				writer.Add($"{KCC.Debug.FixedTimer.Counter}");
				writer.Add($"{KCC.Debug.FixedTimer.GetTotalMilliseconds():F4}");
				writer.Add($"{KCC.Debug.RenderTimer.GetTotalMilliseconds():F4}");
				writer.Add($"{_kcc.PredictionError.magnitude:F4}");
			}
