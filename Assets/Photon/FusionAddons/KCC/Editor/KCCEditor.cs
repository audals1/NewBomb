namespace Fusion.Addons.KCC.Editor
{
	...
	public class KCCEditor : Editor
	{
		...
		private static List<PlayerRef> _replicationList        = new List<PlayerRef>();
		private static bool            _replicationListFoldout = false;
		...
		public override void OnInspectorGUI()
		{
			...
			Color enabledKCCDataColor      = Color.yellow;
			...
			EditorGUILayout.BeginHorizontal();
			{
				EditorGUILayout.BeginVertical();
				{
					...
					if (DrawButton("Use Fixed Data",       kccDebug.UseFixedData       == true, enabledKCCDataColor,         disabledBackgroundColor) == true) { kccDebug.UseFixedData       = true;                         }
				...
				EditorGUILayout.BeginVertical();
				{
					...
					if (DrawButton("Use Render Data",     kccDebug.UseFixedData      == false, enabledKCCDataColor,         disabledBackgroundColor) == true) { kccDebug.UseFixedData      = false;                       }
			...
			EditorGUILayout.LabelField("Replication Mode", kcc.ReplicationMode.ToString());
			...
			EditorGUILayout.LabelField("Collision Queries", $"{kcc.Debug.OverlapQueries.ToString()} / {kcc.Debug.RayCastQueries.ToString()} / {kcc.Debug.ShapeCastQueries.ToString()}");
			...
			if (kcc.ReplicationMode != EKCCReplicationMode.Default)
			{
				if (DrawButtonFoldout($"Replication List ({kcc.ReplicationMode})", ref _replicationListFoldout, enabledBackgroundColor, disabledBackgroundColor) == true)
				{
					kcc.GetReplicationList(_replicationList);
					for (int i = 0; i < _replicationList.Count; ++i)
					{
						PlayerRef     playerRef    = _replicationList[i];
						NetworkObject playerObject = kcc.Runner.GetPlayerObject(playerRef);

						DrawPingDotButton($"{playerRef}", playerObject != null ? playerObject.gameObject : null, foldoutStaticOffset, defaultBackgroundColor);
					}
				}
			}
