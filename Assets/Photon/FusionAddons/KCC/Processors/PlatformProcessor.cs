namespace Fusion.Addons.KCC
{
    ...
    public unsafe class PlatformProcessor : NetworkKCCProcessor, IBeginMove, IEndMove
    {
		// CONSTANTS
		...
		public override void FixedUpdateNetwork()
		{
			...
			if (_kcc.IsPredictingInFixedUpdate == true)
			{
				// This forces sending some data to clients to keep snapshots being constantly updated.
				// Otherwise some visual glitches appear on proxies when data starts changing on state authority.
				++_state.KeepAlive;
		...
		private bool TrySetInterpolatedPosition(KCC kcc, KCCData data)
		{
			...
			bool isPredictingInFixedUpdate = kcc.IsPredictingInFixedUpdate;
			...
			for (int i = 0; i < toState.Platforms.Length; ++i)
			{
				...
				if (fromPlatform.State == EPlatformState.None)
				{
					...
					if (isPredictingInFixedUpdate == true && toPlatform.State != EPlatformState.Active)
						continue;
				...
				else if (toPlatform.State == EPlatformState.None)
				{
					...
					// Render interpolated KCC with predicted fixed simulation - for perfect snapping we want to interpolate only if the state is Active (KCC stands within the platform trigger).
					// Otherwise the KCC could penetrate geometry while keeping Inactive state during platform-space => world-space transition, which is undesired.
					if (isPredictingInFixedUpdate == true && fromPlatform.State != EPlatformState.Active)
						continue;
				...
				else
				{
					...
					// Render interpolated KCC with predicted fixed simulation - for perfect snapping we want to interpolate only if the state is Active (KCC stands within the platform trigger).
					// Otherwise the KCC could penetrate geometry while keeping Inactive state during platform-space => world-space transition, which is undesired.
					if (isPredictingInFixedUpdate == true && (fromPlatform.State != EPlatformState.Active || toPlatform.State != EPlatformState.Active))
						continue;
