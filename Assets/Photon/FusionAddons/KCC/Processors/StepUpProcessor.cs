namespace Fusion.Addons.KCC
{
	...
	public class StepUpProcessor : KCCProcessor, IAfterMoveStep
	{
		...
		public void Execute(AfterMoveStep stage, KCC kcc, KCCData data)
		{
			...
			if (IsTouchingSlopeOrWallOrHang(stage.OverlapInfo) == true)
			{
				tryStepUp = true;
			}
			else
			...
			if (_groundCheckRadiusScale < 1.0f)
			{
				...
				checkPosition += combinedCheckDirectionXZ * (kcc.Settings.Radius - checkRadius);
			}

			// 3. Downward collision check to get step height.
			if (kcc.SphereCast(_shapeCastInfo, checkPosition + new Vector3(0.0f, checkRadius, 0.0f), checkRadius, Vector3.down, maxStepHeight, QueryTriggerInteraction.Ignore) == true)
			{
				...
				maxStepHeight = Mathf.Clamp(maxStepHeight - (checkPosition.y - highestPoint.y) - kcc.Settings.Extent, 0.0f, _stepHeight);
			}
			...
			{
				if (highestPointNormal.IsZero() == false)
				{
		...
		private static bool IsTouchingSlopeOrWallOrHang(KCCOverlapInfo overlapInfo)
		{
