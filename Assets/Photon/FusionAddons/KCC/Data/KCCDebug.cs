namespace Fusion.Addons.KCC
{
	...
	public sealed class KCCDebug
	{
		...
		public KCCTimer       FixedTimer  = new KCCTimer("Fixed");
		public KCCTimer       RenderTimer = new KCCTimer("Render");
		...
		public void BeginFixedUpdate(KCC kcc)
		{
			FixedTimer.Start();
		}

		public void EndFixedUpdate(KCC kcc)
		{
			FixedTimer.Stop();
			...
			if (ShowPath == true)
			{
				...
				if (kcc.IsPredictingInFixedUpdate == true)
				{
		...
		public void BeginRenderUpdate(KCC kcc)
		{
			RenderTimer.Start();
		}

		public void EndRenderUpdate(KCC kcc)
		{
			RenderTimer.Stop();
		...
		public void BeforeUpdate(KCC kcc)
		{
			FixedTimer.Reset();
			RenderTimer.Reset();
		}

		public void BeforeTick(KCC kcc)
		{
			if (UseFixedData == true)
			{
				TraceInfoCount   = default;
		...
		public void AfterTick(KCC kcc)
		{
		}

		public void AfterUpdate(KCC kcc)
		{
			if (UseFixedData == false)
			{
				TraceInfoCount   = default;
				OverlapQueries   = default;
				RayCastQueries   = default;
		...
		public bool TraceStage(Type type, int level, bool isInFixedUpdate)
		{
			if (TraceExecution == false || UseFixedData != isInFixedUpdate)
				return false;
		...
		public bool TraceStage(Type type, string name, int level, bool isInFixedUpdate)
		{
			if (TraceExecution == false || UseFixedData != isInFixedUpdate)
				return false;
