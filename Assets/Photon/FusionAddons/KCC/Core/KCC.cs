namespace Fusion.Addons.KCC
{
	...
	public sealed partial class KCC : NetworkTRSP, IAfterSpawned, IAfterClientPredictionReset, IBeforeUpdate, IBeforeTick, IAfterTick, IAfterUpdate
	{
		// CONSTANTS
		...
		private EKCCReplicationMode _replicationMode       = EKCCReplicationMode.Default;
		private HashSet<int>        _replicationList       = new HashSet<int>();
		...
		private int                 _lastRestoreStateFrame;
		...
		public void Interpolate(RenderSource renderSource, RenderTimeframe renderTimeframe, float alpha = -1.0f)
		{
			...
			OnInterpolateInternal(Data, renderSource, renderTimeframe, alpha);
		}
		...
		void IAfterSpawned.AfterSpawned()
		{
			...
			if (IsPredictingInFixedUpdate == true)
			{
				WriteNetworkData();
			}
		...
		{
			RestoreState(Runner.LatestServerTick);
		}
		...
		// IAfterUpdate INTERFACE

		void IAfterUpdate.AfterUpdate()
		{
			_debug.AfterUpdate(this);

			_renderData.Frame = Time.frameCount;
			_renderData.Tick  = Runner.Tick.Raw;
			...
			{
				_renderData.Time      = Runner.RemoteRenderTime;
				_renderData.DeltaTime = Runner.DeltaTime;
			}

			_renderData.UpdateDeltaTime = _renderData.DeltaTime;

			Trace(nameof(IAfterUpdate.AfterUpdate), "[Render Update Initialization]", $"Time:{_renderData.Time:F6}", $"DeltaTime:{_renderData.DeltaTime:F6}", $"Alpha:{_renderData.Alpha:F4}");
		}

		// PRIVATE METHODS

		private void RestoreState(int tick)
		{
			int frame = Time.frameCount;
			if (_lastRestoreStateFrame == frame)
				return;

			_lastRestoreStateFrame = frame;

			Trace(nameof(RestoreState), $"Tick:{tick}");
			...
			if (IsPredictingInFixedUpdate == true)
			{
				historyData = _historyData[tick % HISTORY_SIZE];
				if (historyData != null && historyData.Tick == tick)
				{
		...
		private void OnFixedUpdateInternal()
		{
			...
			_debug.BeginFixedUpdate(this);

			RefreshCollider();

			if (IsPredictingInFixedUpdate == false)
			{
				if (IsInterpolatingInFixedUpdate == true)
				{
					OnInterpolateInternal(_fixedData, RenderSource.Interpolated, RenderTimeframe.Remote);
				}

				_debug.EndFixedUpdate(this);
				return;
			}
		...
		private void OnRenderUpdateInternal()
		{
			...
			_debug.BeginRenderUpdate(this);

			if (IsPredictingInFixedUpdate == false)
			{
				if (IsInterpolatingInRenderUpdate == true)
				{
					OnInterpolateInternal(_renderData, RenderSource.Interpolated, RenderTimeframe.Remote);
				}

				_lastRenderPosition = _renderData.TargetPosition;
				_lastRenderTime     = _renderData.Time;

				_debug.EndRenderUpdate(this);
				return;
			}
			...
			else if (IsInterpolatingInRenderUpdate == true)
			{
		...
		private void OnInterpolateInternal(KCCData data, RenderSource renderSource, RenderTimeframe renderTimeframe, float alpha = -1.0f)
		{
			InterpolateNetworkData(data, renderSource, renderTimeframe, alpha);

			if (data.IsActive == true)
			{
				CacheProcessors(data);
				InvokeOnInterpolate(data);
				SynchronizeTransform(data, true, true, false);
			}
		}
		...
		private void SetDefaults(bool cleanup)
		{
			...
			_lastRestoreStateFrame  = default;
			_predictionError        = default;
			...
			if (cleanup == true)
			{
				...
				_replicationMode = default;

				_settings.CopyFromOther(_defaultSettings);

				_collider.Destroy();

				SetReplicationMode(EKCCReplicationMode.Default, true);
