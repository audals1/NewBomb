namespace Fusion.Addons.KCC
{
	...
	public partial class KCC
	{
		...
		/// <summary>
		/// Executes stage of type <c>IKCCStage&lt;TStageObject&gt;</c> with the stage object of type <c>TStageObject</c> on all processors.
		/// </summary>
		...
		/// </summary>
		public void ExecuteStage<TStage, TStageObject>(TStageObject stage) where TStage : IKCCStage<TStageObject> where TStageObject : IKCCStage<TStageObject>
		{
		...
		{
			if (_activeStages.Count == 0)
				throw new InvalidOperationException("Querying processor pending execution is allowed only during stage execution!");

			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				KCCStageInfo activeStageInfo = _activeStages[i];
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					if (activeStageInfo.HasPendingProcessor(processor) == true)
						return true;
				}
			}
		...
		public bool HasPendingProcessor<TProcessor, TStage>() where TProcessor : class where TStage : IKCCStage
		{
			...
			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					if (activeStageInfo.HasPendingProcessor<TProcessor>() == true)
						return true;
				}
			}
		...
		public TProcessor GetPendingProcessor<TProcessor>() where TProcessor : class
		{
			...
			if (_activeStages[_activeStages.Count - 1].GetPendingProcessor<TProcessor>(out TProcessor processor) == true)
				return processor;
		...
		public TProcessor GetPendingProcessor<TProcessor, TStage>() where TProcessor : class where TStage : IKCCStage
		{
			...
			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					if (activeStageInfo.GetPendingProcessor<TProcessor>(out TProcessor processor) == true)
						return processor;
				}
			}
		...
		{
			if (_activeStages.Count == 0)
				throw new InvalidOperationException("Querying processors pending execution is allowed only during stage execution!");

			_activeStages[_activeStages.Count - 1].GetPendingProcessors<TProcessor>(processors, true);
		}
		...
		/// The query does NOT include the currently executed processor.
		/// </summary>
		public void GetPendingProcessors<TProcessor, TStage>(List<TProcessor> processors) where TProcessor : class where TStage : IKCCStage
		{
			if (_activeStages.Count == 0)
				throw new InvalidOperationException("Querying processors pending execution is allowed only during stage execution!");
			...
			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					activeStageInfo.GetPendingProcessors<TProcessor>(processors, false);
				}
			}
		}
		...
		{
			if (_activeStages.Count == 0)
				throw new InvalidOperationException("Querying executed processor is allowed only during stage execution!");
		...
		public bool HasExecutedProcessor<TStage>(IKCCProcessor processor) where TStage : IKCCStage
		{
			...
			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					if (activeStageInfo.HasExecutedProcessor(processor) == true)
						return true;
				}
			}
		...
		public bool HasExecutedProcessor<TProcessor, TStage>() where TProcessor : class where TStage : IKCCStage
		{
			...
			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					if (activeStageInfo.HasExecutedProcessor<TProcessor>() == true)
						return true;
				}
			}
		...
		public TProcessor GetExecutedProcessor<TProcessor>() where TProcessor : class
		{
			...
			if (_activeStages[_activeStages.Count - 1].GetExecutedProcessor<TProcessor>(out TProcessor processor) == true)
				return processor;
		...
		public TProcessor GetExecutedProcessor<TProcessor, TStage>() where TProcessor : class where TStage : IKCCStage
		{
			...
			for (int i = _activeStages.Count - 1; i >= 0; --i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					if (activeStageInfo.GetExecutedProcessor<TProcessor>(out TProcessor processor) == true)
						return processor;
				}
			}
		...
		{
			if (_activeStages.Count == 0)
				throw new InvalidOperationException("Querying executed processors is allowed only during stage execution!");

			_activeStages[_activeStages.Count - 1].GetExecutedProcessors<TProcessor>(processors, true);
		}
		...
		/// Processors in the list are sorted by priority.
		/// </summary>
		public void GetExecutedProcessors<TProcessor, TStage>(List<TProcessor> processors) where TProcessor : class where TStage : IKCCStage
		{
			if (_activeStages.Count == 0)
				throw new InvalidOperationException("Querying executed processors is allowed only during stage execution!");
			...
			for (int i = 0, count = _activeStages.Count; i < count; ++i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					activeStageInfo.GetExecutedProcessors<TProcessor>(processors, false);
				}
			}
		}
		...
		public bool SuppressProcessor(IKCCProcessor processor, bool suppressInFutureStages = false)
		{
			...
			bool result = _activeStages[_activeStages.Count - 1].SuppressProcessor(processor);

			if (suppressInFutureStages == true)
			{
				for (int i = 0, count = _stageProcessorCount; i < count; ++i)
				{
					if (ReferenceEquals(_stageProcessors[i], processor) == true)
					{
		...
		public bool SuppressProcessor<TStage>(IKCCProcessor processor) where TStage : IKCCStage
		{
			...
			for (int i = 0, count = _activeStages.Count; i < count; ++i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					result |= activeStageInfo.SuppressProcessor(processor);
				}
			}
		...
		public bool SuppressProcessors<TProcessor>(bool suppressInFutureStages = false) where TProcessor : class
		{
			...
			bool result = _activeStages[_activeStages.Count - 1].SuppressProcessors<TProcessor>();
		...
		public bool SuppressProcessors<TProcessor, TStage>() where TProcessor : class where TStage : IKCCStage
		{
			...
			for (int i = 0, count = _activeStages.Count; i < count; ++i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					result |= activeStageInfo.SuppressProcessors<TProcessor>();
				}
			}
		...
		/// <summary>
		/// Suppress execution of all pending processors except processors of type <c>TProcessor</c> in all active stages of type <c>TStage</c>.
		/// </summary>
		public void SuppressProcessorsExcept<TProcessor, TStage>() where TProcessor : class where TStage : IKCCStage
		{
			...
			for (int i = 0, count = _activeStages.Count; i < count; ++i)
			{
				...
				if (activeStageInfo.IsAssignable<TStage>() == true)
				{
					activeStageInfo.SuppressProcessorsExcept<TProcessor>();
				}
			}
		}
		...
		public void EnqueuePostProcess(Action<KCC, KCCData> callback)
		{
			...
			KCCStageInfo activeStageInfo = _activeStages[_activeStages.Count - 1];
		...
		/// <summary>
		/// Enforcing execution of a specific KCC feature. The call is valid only with BeginMove being the first stage in hierarchy.
		...
		public void EnforceFeature(EKCCFeature feature)
		{
			if (_activeStages.Count == 0 || _activeStages[0].StageObjectType != KCCTypes.BeginMove)
				throw new InvalidOperationException($"Enforcing features is allowed only during {nameof(BeginMove)} stage execution!");

			_activeFeatures = (EKCCFeatures)((int)_activeFeatures | (1 << (int)feature));
		}

		/// <summary>
		/// Suppressing execution of a specific KCC feature. The call is valid only with BeginMove being the first stage in hierarchy.
		/// For persistent change please set KCC.Settings.Features directly.
		/// </summary>
		public void SuppressFeature(EKCCFeature feature)
		{
			if (_activeStages.Count == 0 || _activeStages[0].StageObjectType != KCCTypes.BeginMove)
				throw new InvalidOperationException($"Suppressing features is allowed only during {nameof(BeginMove)} stage execution!");
		...
		private void ExecuteStageInternal<TStage, TStageObject>(TStageObject stage) where TStage : IKCCStage<TStageObject> where TStageObject : IKCCStage<TStageObject>
		{
			if (_isSpawned == false)
				return;

			Type type = typeof(TStageObject);
			if (type == KCCTypes.IBeginMove || type == KCCTypes.BeginMove)
				throw new InvalidOperationException($"Explicit execution of {nameof(IBeginMove)} stage is not allowed!");
			if (type == KCCTypes.IPrepareData || type == KCCTypes.PrepareData)
				throw new InvalidOperationException($"Explicit execution of {nameof(IPrepareData)} stage is not allowed!");
			if (type == KCCTypes.IAfterMoveStep || type == KCCTypes.AfterMoveStep)
				throw new InvalidOperationException($"Explicit execution of {nameof(IAfterMoveStep)} stage is not allowed!");
		...
		private void ExecuteStageInternal<TStage, TStageObject>(TStageObject stage, KCCData data) where TStage : IKCCStage<TStageObject> where TStageObject : IKCCStage<TStageObject>
		{
			KCCStageInfo stageInfo = KCCStageInfo.Allocate();
			...
			IKCCProcessor[]   stageProcessors    = stageInfo.Processors;
			List<PostProcess> stagePostProcesses = stageInfo.PostProcesses;

			for (int i = 0; i < _stageProcessorCount; ++i)
			{
				IKCCProcessor stageProcessor = _stageProcessors[i];
			...
			bool traceProcessors = _debug.TraceStage(stageInfo.StageObjectType, stageInfo.Level, IsInFixedUpdate);
			...
			for (stageInfo.ProcessorIndex = 0; stageInfo.ProcessorIndex < stageInfo.ProcessorCount; ++stageInfo.ProcessorIndex)
			{
				IKCCProcessor processor = stageProcessors[stageInfo.ProcessorIndex];
				if (object.ReferenceEquals(processor, null) == true)
					continue;

				if (traceProcessors == true)
				{
					_debug.TraceProcessor(processor, stageInfo.Level);
				}
			...
			for (int i = 0; i < stagePostProcesses.Count; ++i)
			{
				try
				{
			...
			int lastStageIndex = _activeStages.Count - 1;
			...
			{
				LogError($"Active stage of type {stageInfo.Type.FullName} is at unexpected position ({lastStageIndex}) in execution stack!");
		...
		private void InvokeOnStay(KCCData data)
		{
			...
			bool traceProcessors = _debug.TraceStage(default, nameof(IKCCProcessor.OnStay), stageInfo.Level, IsInFixedUpdate);

			for (int i = 0; i < _cachedProcessorCount; ++i)
			{
				IKCCProcessor processor = _cachedProcessors[i];
				if (object.ReferenceEquals(processor, null) == true)
					continue;
			...
			{
				LogError($"Active stage of type {stageInfo.Type.FullName} is at unexpected position in execution stack!");
				_activeStages.Remove(stageInfo);
			}
		...
		private void InvokeOnInterpolate(KCCData data)
		{
			...
			bool traceProcessors = _debug.TraceStage(default, nameof(IKCCProcessor.OnInterpolate), stageInfo.Level, IsInFixedUpdate);
